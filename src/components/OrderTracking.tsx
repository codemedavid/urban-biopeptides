...
import { Search, Package, Truck, CheckCircle, Clock, AlertCircle, ArrowRight, ArrowLeft, ExternalLink } from 'lucide-react';
...
const OrderTracking: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-white via-gold-50/10 to-white">
                ...
                <div className="container mx-auto px-4 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-xl md:text-2xl font-bold text-gray-900">Track Your Order</h1>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="py-12 px-4 sm:px-6 lg:px-8">
                <div className="max-w-3xl mx-auto">
                    ...
                    {/* Search Box */}
                    <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-8 border-2 border-teal-400/30">
                        <form onSubmit={handleTrack} className="flex flex-col md:flex-row gap-4">
                            <div className="flex-1 relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-3 rounded-xl border-2 border-gray-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-400/20 outline-none transition-all text-lg"
                                />
                            </div>
                            <button
                                type="submit"
                    ...
                    {hasSearched && order && (
                        <div className="space-y-6 animate-fade-in">
                            {/* Status Card */}
                            <div className="bg-white rounded-2xl shadow-xl border-2 border-teal-400/30 overflow-hidden">
                                <div className="bg-teal-500 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 text-white">
                                    <div>
                                        ...
                                        <h2 className="text-2xl font-bold capitalize flex items-center gap-2 text-white">
                                            ...
                                            {order.order_status === 'confirmed' && <CheckCircle className="w-6 h-6 text-pink-200" />}
                                            {order.order_status === 'processing' && <Package className="w-6 h-6 text-teal-200" />}
                                            {order.order_status === 'shipped' && <Truck className="w-6 h-6 text-green-300" />}
                                            {order.order_status === 'delivered' && <CheckCircle className="w-6 h-6 text-green-300" />}
                                            {order.order_status === 'cancelled' && <AlertCircle className="w-6 h-6 text-red-300" />}
                                            {order.order_status}
                                        </h2>
                                    </div>
                                    ...
                                    {order.order_status !== 'cancelled' ? (
                                        <div className="mb-8">
                                            <div className="relative">
                                                ...
                                                <div
                                                    className="absolute top-1/2 left-0 h-1 bg-pink-400 -translate-y-1/2 rounded-full transition-all duration-500"
                                                    ...
                                                    {['Placed', 'Confirmed', 'Processing', 'Shipped', 'Delivered'].map((step, index) => {
                                                        ...
                                                        return (
                                                            <div key={step} className="flex flex-col items-center gap-2">
                                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all duration-300 bg-white ${isCompleted ? 'border-teal-500 text-teal-500' : 'border-gray-300 text-gray-300'
                                                                    } ${isCurrent ? 'ring-4 ring-pink-300/40 scale-110' : ''}`}>
                                                                    {index < currentStep ? (
                                                                        <CheckCircle className="w-5 h-5 fill-pink-50" />
                                                                    ) : (
                                                                        <div className={`w-3 h-3 rounded-full ${isCompleted ? 'bg-teal-500' : 'bg-gray-300'}`} />
                                                                    )}
                                                                </div>
                                            ...
                                            {order.tracking_number ? (
                                                <div className="space-y-4">
                                                    <div>
                                                        ...
                                                        <p className="text-xl font-mono font-bold text-gray-900 tracking-wide">{order.tracking_number}</p>
                                                    </div>
